 ...
 * @Description:
 */
public class ChangePhonePresenter extends BasePresenter implements ChangePhoneContract.Presenter {

    private              MainDataManager          mDataManager;
    private              ChangePhoneContract.View mLoginView;
    private static final String                   TAG = "ChangePasswordPresenter";

    @Inject
    public ChangePhonePresenter(MainDataManager mDataManager, ChangePhoneContract.View view) {
        ...
        Disposable disposable = mDataManager.getForgetPasswordData(mapHeaders, mapParameters, new ErrorDisposableObserver<ResponseBody>() {
            ...
            public void onNext(ResponseBody responseBody) {
                try {
                    ...
                    Gson gson = new Gson();
                    ChangePhoneResponse changePhoneResponse = gson.fromJson(response, ChangePhoneResponse.class);

                    mLoginView.setResponseData(changePhoneResponse);
                } catch (Exception e) {
                    e.printStackTrace();
                }
