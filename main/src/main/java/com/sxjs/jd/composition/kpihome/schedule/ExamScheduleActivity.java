 ...
 * @author xiepeng
 */
@Route(path = "/examScheduleActivity/examScheduleActivity")
public class ExamScheduleActivity extends BaseActivity implements ExamScheduleContract.View, PtrHandler {
    @Inject
    ExamSchedulePresenter mPresenter;
    ...
    private              List<ExamScheduleResponse.DataBean> mListData;
    ...
    private void initView() {
        DaggerExamScheduleActivityComponent.builder()
                .appComponent(getAppComponent())
                .examSchedulePresenterModule(new ExamSchedulePresenterModule(this, MainDataManager.getInstance(mDataManager)))
                .build()
                .inject(this);
    ...
    @Override
    public void setResponseData(ExamScheduleResponse examScheduleResponse) {
        this.examScheduleResponse = examScheduleResponse;
        try {
            String code = examScheduleResponse.getCode();
            String msg = examScheduleResponse.getMsg();
            if (code.equals(ResponseCode.SUCCESS_OK)) {
                List<ExamScheduleResponse.DataBean> data = examScheduleResponse.getData();
                mListData.clear();
    ...
    class MyAdapter extends BaseAdapter {
        ...
        public MyAdapter(List<ExamScheduleResponse.DataBean> mListData) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            ExamScheduleResponse.DataBean dataBean = mListData.get(position);

            String status = dataBean.getEXAM_STAGE_STATUS();
            //            进行状态0未开始1-进行中2-已结束
            ...
            String exam_stage_name = dataBean.getEXAM_STAGE_NAME();
            if (!TextUtils.isEmpty(exam_stage_name)) {
                holder.title.setText(exam_stage_name);
            }
            String exam_stage_end_date = dataBean.getEXAM_STAGE_END_DATE();
