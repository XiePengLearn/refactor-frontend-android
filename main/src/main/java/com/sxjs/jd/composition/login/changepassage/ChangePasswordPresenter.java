...
public class ChangePasswordPresenter extends BasePresenter implements ChangePasswordContract.Presenter {
    ...
    private              ChangePasswordContract.View mLoginView;
    private static final String                      TAG = "ChangePasswordPresenter";

    @Inject
    public ChangePasswordPresenter(MainDataManager mDataManager, ChangePasswordContract.View view) {
    ...
    public void getRequestData(Map<String, String> mapHeaders, Map<String, Object> mapParameters) {
        ...
        Disposable disposable = mDataManager.getChangePasswordData(mapHeaders, mapParameters, new ErrorDisposableObserver<ResponseBody>() {
            @Override
            public void onNext(ResponseBody responseBody) {
                try {
                    ...
                    Gson gson = new Gson();
                    ChangePasswordResponse changePasswordResponse = gson.fromJson(response, ChangePasswordResponse.class);

                    mLoginView.setResponseData(changePasswordResponse);
                } catch (Exception e) {
                    e.printStackTrace();
                }
